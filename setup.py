...
PYBIND_DIR = os.path.join(SRC_DIR, "pybind_processor")
COMMON_DIR = os.path.join(SRC_DIR, "common")

# Cython source file
PYX_FILE = os.path.join(CYTHON_DIR, "cython_processor.pyx")

# pybind11 source file
PYBIND_FILE = os.path.join(PYBIND_DIR, "pybind_processor.cpp")
...
pybind_ext_modules = [
    Extension(
        "pybind_processor.pybind_processor",  # Full package path for module
        sources=[PYBIND_FILE, CPP_FILE],  # Combines pybind11 wrapper with C++ implementation
        include_dirs=[
            numpy.get_include(),
            pybind11.get_include(),
            SRC_DIR,            # Include src directory
            COMMON_DIR,         # Include common directory for C++ headers
            PYBIND_DIR,         # Include pybind directory
            ".",                # Include root directory
        ],
        language="c++",
        define_macros=[
        ...
        extra_compile_args=["-std=c++11", "-Wno-unreachable-code"],
    )
]
...
# Combine all extension modules
# Cython modules need to be processed by cythonize, pybind11 modules don't
all_ext_modules = cythonize(
    cython_ext_modules,
    build_dir=BUILD_DIR,
